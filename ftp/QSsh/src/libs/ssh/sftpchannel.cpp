...
SftpJobId SftpChannel::realPath(const QString &path)
{
    return d->createJob(Internal::SftpRealPath::Ptr(
        new Internal::SftpRealPath(++d->m_nextJobId, path)));
}
...
void SftpChannelPrivate::handleName()
{
    ...
    case AbstractSftpOperation::RealPath: {
        SftpRealPath::Ptr op=it.value().dynamicCast<SftpRealPath>();
        if(op){
            SftpFileInfo fileInfo;
            fileInfo.name = response.files[0].fileName;
