...
struct AbstractSftpOperation
{
    ...
    enum Type {
        StatFile, ListDir, MakeDir, RmDir, Rm, Rename, CreateLink, CreateFile, Download, UploadFile,RealPath
    };
...
//added by whg,2020.4.1
struct  SftpRealPath : public AbstractSftpOperation
{
    typedef QSharedPointer<SftpRealPath> Ptr;

    SftpRealPath(SftpJobId jobId, const QString &path);
    virtual Type type() const { return RealPath; }
    virtual SftpOutgoingPacket &initialPacket(SftpOutgoingPacket &packet);
